...
export class AiExpertSuite implements ExpertSuite {
  ...
  reviewTarget(input: TargetedReviewInput): Promise<ExpertResult<TargetedReview>> {
    return this.dependencies.generator.generate({
      ...
      }))}\n\n请回应用户对目标条目的质疑；仅在证据要求修改时返回 replacement。replacement 只能修改 target 指定的一个条目，currentModule 的其他字段、条目与顺序必须原样保留。不得新增来源或改写来源元数据；所有 sourceId 和 newEvidenceSourceIds 只能引用 persistedSources 中已有的 id。`,

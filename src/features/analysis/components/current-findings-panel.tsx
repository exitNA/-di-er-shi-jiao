...
export function CurrentFindingsPanel({
  ...
  return <section className="flex h-full min-h-0 flex-col" aria-labelledby="current-findings-heading">
    <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-xs font-medium tracking-[0.16em] text-secondary">AGENT CANVAS</p><h2 id="current-findings-heading" className="mt-2 font-display text-2xl font-semibold">当前思考画布</h2></div><p className="pt-5 text-xs text-ink-faint">点击带入对话</p></div>
    ...
    <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">{findings.length ? findings.map((finding) => <button key={`${finding.target.moduleType}-${finding.target.section}-${finding.target.itemId}`} type="button" onClick={() => onSelect(finding.target)} className={`w-full rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md ${sameTarget(selectedTarget, finding.target) ? "border-secondary bg-mist/65" : "border-border bg-white/80"}`}><p className={`text-xs font-semibold ${finding.tone}`}>{finding.label}</p><p className="mt-2 leading-7 text-ink">{finding.text}</p><span className="mt-3 block text-xs text-secondary">继续追问 →</span></button>) : <div className="rounded-2xl border border-dashed border-border p-5 text-sm leading-6 text-ink-faint">Agent 正在整理这一层发现。你可以继续补充材料或调整问题。</div>}</div>
  </section>;
}

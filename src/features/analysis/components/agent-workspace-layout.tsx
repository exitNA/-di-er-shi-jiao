...
export function AgentWorkspaceLayout({
  ...
  function updateWidth(clientX: number) {
    ...
    if (!bounds) return;
    setLeftWidth(Math.min(58, Math.max(28, Math.round(((clientX - bounds.left) / bounds.width) * 100))));
  }
  ...
  return (
    <div ref={container}>
      {!isDesktop ? <><div className="mb-3 flex rounded-xl border border-border bg-white p-1" role="tablist" aria-label="工作区视图">
      ...
      </div><div>{mobileView === "conversation" ? conversation : findings}</div></> : <div className="grid min-h-[calc(100vh-9rem)]" style={{ gridTemplateColumns: `${leftWidth}fr 13px ${100 - leftWidth}fr` }}>
        <div className="min-w-0 pr-6">{conversation}</div>
        <div role="separator" tabIndex={0} aria-label="调整对话与发现区域宽度" aria-orientation="vertical" aria-valuemin={28} aria-valuemax={58} aria-valuenow={leftWidth} className="group relative cursor-col-resize outline-none" onKeyDown={resizeWithKeyboard} onPointerDown={startResize} onPointerMove={(event) => event.currentTarget.hasPointerCapture(event.pointerId) && updateWidth(event.clientX)}>
        ...
        <div className="min-w-0 pl-6">{findings}</div>
      </div>}
    </div>
  );
}

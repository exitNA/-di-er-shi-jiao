...
import { commonSystemInstruction } from "@/server/agents/prompts/common";
...
function generator(sourceValue = { claims: [statement], sources: [], relations: [], gaps: [] }): StructuredGenerator & { generate: ReturnType<typeof vi.fn> } {
  return {
...
describe("AiExpertSuite", () => {
  it("puts every expert prompt behind Chinese safety instructions and encoded data boundaries", async () => {
    ...
    const outputs = { argument: draft.argument, perspectives: draft.perspectives, sources: draft.sources, risks: draft.risks };
    ...
    for (const [input] of model.generate.mock.calls) {
      expect(input.system).toContain(commonSystemInstruction);
  ...
  it("runs at concurrency two, caps search inputs, ranks tiers first, and deduplicates registrable domains", async () => {
    ...
    const search: SearchClient & { search: ReturnType<typeof vi.fn> } = {
      search: vi.fn(async ({ query }) => {
        ...
        active -= 1;
        return [
          result("https://commercial.example.com/article", 0.99),
          result("https://data.example.gov/report", 0.1),
          result("https://one.example.co.uk/a", 0.2),
          result("https://two.example.co.uk/b", 0.9),
          result(`https://${query.length}.independent.example.net/report`, 0.7),
        ];
      }),
    };
    const suite = new AiExpertSuite({ generator: model, searchClient: search });
    ...
    expect(prompt.indexOf("data.example.gov")).toBeLessThan(prompt.indexOf("commercial.example.com"));
  ...
  it("selects three to five real candidates when available and replaces invalid sparse-source gaps", async () => {
    ...
    const sparseModel = generator({ claims: [statement], sources: [], relations: [], gaps: [{ ...statement, id: "bad-gap", origin: "ai_inference", text: "资料稍后再看。" }] });
